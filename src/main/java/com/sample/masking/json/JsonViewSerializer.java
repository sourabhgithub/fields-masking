...
public class JsonViewSerializer extends JsonSerializer<JsonView> {
  ...
  public <T> void registerCustomSerializer(Class<T> cls, JsonSerializer<T> forType) {
    ...
    } else if(cls.equals(com.sample.masking.json.JsonView.class)) {
      throw new IllegalArgumentException("Class cannot be " + com.sample.masking.json.JsonView.class);
  ...
  /**
   * Set the default matcher behavior to be used if the {@link com.sample.masking.json.JsonView} object to
  ...
  @Override
  public void serialize(com.sample.masking.json.JsonView result, JsonGenerator jgen, SerializerProvider serializers) throws IOException {
    new JsonViewSerializer.JsonWriter(serializers, jgen, result).write(null, result.getValue());
  }
  ...
  class JsonWriter {
    ...
    JsonViewSerializer.AccessibleProperty referringField = null;

    final SerializerProvider serializerProvider;
    final JsonGenerator jgen;
    final com.sample.masking.json.JsonView result;

    JsonWriter(SerializerProvider serializerProvider, JsonGenerator jgen, com.sample.masking.json.JsonView result) {
    ...
    boolean writeList(Object obj) throws IOException {
      if(obj instanceof List || obj instanceof Set || obj.getClass().isArray()) {
        ...
        for(Object o : iter) {
          new JsonViewSerializer.JsonWriter(jgen, result, currentMatch, currentPath, path, referringField, serializerProvider).write(null, o);
        }
        jgen.writeEndArray();
      } else {
        return false;
    ...
    boolean writeMap(Object obj) throws IOException {
      if(obj instanceof Map) {
        ...
        for(Object key : map.keySet()) {
          ...
          new JsonViewSerializer.JsonWriter(jgen, result, currentMatch, serializerProvider).write(null, map.get(key));
        }
        jgen.writeEndObject();
      } else {
        return false;
    ...
    void writeObject(Object obj) throws IOException {
      ...
      List<JsonViewSerializer.AccessibleProperty> fields = getAccessibleProperties(obj.getClass());

      for(JsonViewSerializer.AccessibleProperty property : fields) {
        try {
          ...
          if(fieldAllowed(property, obj.getClass())) {
            ...
            if(fieldSerializer != null) {
              ...
              if(serializer != null) {
                ...
                new JsonViewSerializer.JsonWriter(jgen, result, currentMatch, currentPath, path, property, serializerProvider).write(name, val);
              }
    ...
    boolean valueAllowed(JsonViewSerializer.AccessibleProperty property, Object value, Class cls) {
...
//      });
    ...
    @SuppressWarnings("unchecked")
    boolean fieldAllowed(JsonViewSerializer.AccessibleProperty property, Class declaringClass) {
      String name = property.name;
      if(Modifier.isStatic(property.modifiers)) {
        return false;
      }

      JsonViewSerializer.MatchPrefixTuple tuple = getMatchPrefix(declaringClass);
    ...
    JsonViewSerializer.MatchPrefixTuple getMatchPrefix(Class declaringClass) {
      ...
      return new JsonViewSerializer.MatchPrefixTuple(match, prefix);
    }

    Object readField(Object obj, JsonViewSerializer.AccessibleProperty field) throws IllegalAccessException {
      JsonViewSerializer.MatchPrefixTuple tuple = getMatchPrefix(obj.getClass());
    ...
    JsonSerializer annotatedWithJsonSerialize(JsonViewSerializer.AccessibleProperty property) {
    ...
    private List<JsonViewSerializer.AccessibleProperty> getAccessibleProperties(Class cls) {
      return memoizer.accessibleProperty(cls, () -> {
        // Gather all fields and methods
        Map<String, JsonViewSerializer.AccessibleProperty> accessibleProperties = new LinkedHashMap<>();
        ...
        getDeclaredFields(cls).stream()
                .map(f -> new JsonViewSerializer.AccessibleProperty(f.getName(), f.getAnnotations(), f))
        ...
        getDeclaredMethods(cls).stream()
                ...
                .map(m -> new JsonViewSerializer.AccessibleProperty(getFieldNameFromGetter(m), m.getAnnotations(), m))
                .forEach(p -> {
                  JsonViewSerializer.AccessibleProperty field = accessibleProperties.get(p.name);
                  ...
                  if(field != null) {
                    ...
                    p = new JsonViewSerializer.AccessibleProperty(p.name, annotations.toArray(new Annotation[0]), p.property);
                  }
    ...
    @SuppressWarnings("unchecked")
    private <T extends Annotation> T getAnnotation(JsonViewSerializer.AccessibleProperty property, Class<T> annotation) {
    ...
    private String getFieldName(JsonViewSerializer.AccessibleProperty property) {
  ...
  static class AccessibleProperty {
    ...
    public boolean equals(Object o) {
      ...
      JsonViewSerializer.AccessibleProperty that = (JsonViewSerializer.AccessibleProperty) o;
